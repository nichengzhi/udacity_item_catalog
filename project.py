...
def restaurantsJSON():
    ...
    for cat in tempdata:
        cat["Item"] =[x.serialize for x in session.query(Item).filter_by(catagory_id = cat["id"]).all()]
...
def gconnect():
    ...
    try:
        ...
        credentials = oauth_flow.step2_exchange(code)#use code from google get user's access token
...
def showcatagories():
    ...
    itemswithcat = [session.query(Category).filter_by(id=item.catagory_id).one() for item in items]
    print(session.query(Category).filter_by(id=items[0].catagory_id).one().name)
    return render_template('catagories.html', catagories = catagories, items = zip(items, itemswithcat))
...
def showItem(catagory):
    ...
    if 'username' not in login_session:
        return render_template('publicitem.html', catagories = catagories, items = items, catagory = catagory, total = total)
    else:
        return render_template('item.html', catagories = catagories, items = items, catagory = catagory, total = total)
...
def showDescription(catagory_name, item_name):
    ...
    if login_session['user_id'] == creator_id:
        ...
        return render_template('description.html', item = item, catagory = catagory)
    else:
        return render_template('publicdescription.html', item = item, catagory = catagory)
...
def editItem(item):
    ...
    if request.method == 'POST':
        ...
        if request.form['catagory']:
            cat_id = session.query(Category).filter_by(name = request.form['catagory']).one().id
            #print(cat_id)
        ...
        return render_template('edit.html', item = item, catagory = catagory, total_cat = total_cat)
...
def addItem(catagory_name):
    ...
    if request.method == 'POST':
        if request.form['name'] and request.form['description']:
            newitem = Item(name = request.form['name'], description = request.form['description'], catagory_id = catagory.id, user_id = login_session['user_id'])
            session.add(newitem)
            session.commit()
